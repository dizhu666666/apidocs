...
<div class="form-content mt-10"><table   class="form" border="0">
  <tr>
	...
	<label>接口名称：</label>
	    ...
	    <td><input type="text" name="title" value="${record.title }" class="input1 wd2" /><span class="gray_9"></span></td><td></td>
  </tr>
  <tr>
...
<label>prefix：</label>
    ...
    <td><input type="text" name="content" value="${record.content }" class="input1 wd2" /><span class="gray_9">http://www.baidu.com</span></td>
  </tr>
  <tr>
   <td class="label-td">
<label>路径url：</label>
</td>
    <td><input type="text" name="url" value="${record.url }" id="textfield" class="input1 wd2" /><span class="gray_9">/live/living</span></td>
  </tr>
  <tr>
  <td class="label-td">
<label>访问方式：</label>
  ...
  <c:if test="${recordTwo.type==0 }">
   ...
   <td class="label-td"> </td>
    <td> <input type="text" name="co_title" id="textfield" value="${recordTwo.coTitle }" class="input1 wd2" />
      <select name="co_type" id="select" class="select1 wd4">
             <option value="int" <c:if test="${recordTwo.coType.equals('int')}">selected="true"</c:if>   >int</option>
             <option value="float"<c:if test="${recordTwo.coType.equals('float')}">selected="true"</c:if>  >float</option>
             <option value="double" <c:if test="${recordTwo.coType.equals('double')}">selected="true"</c:if> >double</option>
             <option value="long"<c:if test="${recordTwo.coType.equals('long')}">selected="true"</c:if> >long</option>
             <option value="string"<c:if test="${recordTwo.coType.equals('string')}">selected="true"</c:if> >string</option>
              <option value="array"<c:if test="${recordTwo.coType.equals('array')}">selected="true"</c:if> >array</option>
             <option value="object"<c:if test="${recordTwo.coType.equals('object')}">selected="true"</c:if> >object</option>
        </select>
    <input type="text" name="co_note" id="textfield" value="${recordTwo.coNote }" class="input1 wd2" /><input type="button" onclick="rmline(this)"  class="btn btn-outline btn-primary variable" value="删除" /> </td>
  </tr>
  </c:if>
  </c:forEach>
</table></div></div>
...
<div class="form-content mt-10"><table id="restab"  class="form" border="0">
  ...
  <c:if test="${recordTwo.type==1 }">
   <tr class="column">
   <td class="label-td"> </td>
    <td> <input type="text" name="co_title" value="${recordTwo.coTitle }" id="textfield" class="input1 wd2" />
      <select name="co_type" id="select" class="select1 wd4">
             <option value="int" <c:if test="${recordTwo.coType.equals('int')}">selected="true"</c:if>>int</option>
             <option value="float"<c:if test="${recordTwo.coType.equals('float')}">selected="true"</c:if>  >float</option>
             <option value="double" <c:if test="${recordTwo.coType.equals('double')}">selected="true"</c:if> >double</option>
